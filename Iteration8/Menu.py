...
class Button():
    def __init__(self,y_pos,size,text):
        self.size = size
...
def Menu1(window,surface):
    ...
    button_size = 70
    button_list = []
    button_list.append(Button(SIZE[1]//3.5,button_size,'Single Player'))
    button_list.append(Button((SIZE[1]//3.5) * 2,button_size,'Two Player'))
    ...
    while not choice:
        ...
        for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if button_list[0].rect.collidepoint(mouse):
                        choice = True
                        return 'single'
                    elif button_list[1].rect.collidepoint(mouse):
                        choice = True
                        return 'double'
